...
import { Target, Eye, Compass, Lightbulb, ArrowRight } from 'lucide-react'
...
const STRATEGIC_GOALS = [
  {
    ...
    description: 'Ensure 50,000 children per year receive quality foundational education, vocational training, or scholarship support by 2028.',
    metric: '50,000 learners/year',
  },
  {
    number: '02',
    title: 'Strengthen Community Healthcare',
    description: 'Deliver free preventive and primary healthcare to 100,000 individuals annually through mobile clinics and community health workers.',
    metric: '100,000 patients/year',
  },
  {
    number: '03',
    title: 'Scale Mentorship Networks',
    description: 'Connect 10,000 young people per year with trained mentors across 15 African countries through in-person and digital platforms.',
    metric: '10,000 mentees/year',
  },
  {
    number: '04',
    title: 'Lead Environmental Action',
    description: 'Plant 1 million trees, restore 5 degraded ecosystems, and provide clean water access to 50 communities by 2027.',
    metric: '1M trees by 2027',
  },
  {
    number: '05',
    title: 'Build Emergency Resilience',
    description: 'Maintain 72-hour emergency response capacity to reach disaster-affected communities within AOF\'s operational geography.',
      ...
      <section className="section">
        <div className="container-xl">
          <div className="grid lg:grid-cols-2 gap-8">
            <Card className="p-8 border-l-4 border-brand-600">
        ...
        <div className="container-xl">
          <div className="text-center mb-12">
            ...
            <p className="text-body max-w-2xl mx-auto mt-4 text-neutral-600 dark:text-neutral-400">
              Ten values that are not aspirational posters on a wall — they are tested, lived, and held accountable at every level of the Foundation.
            </p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-6">
            {CORE_VALUES.map((value, i) => (
              <Card key={value.name} className="p-6 group hover:border-brand-300 hover:shadow-card-hover transition-all duration-300">
                <div className="text-4xl mb-3 group-hover:scale-110 transition-transform duration-200">{value.icon}</div>
                <h3 className="font-display font-bold text-base text-neutral-900 dark:text-neutral-100 mb-2">{value.name}</h3>
                <p className="text-sm text-neutral-600 dark:text-neutral-400 leading-relaxed">{value.description}</p>
                <div className="mt-4 text-xs font-mono text-brand-400">{String(i + 1).padStart(2, '0')}</div>
              </Card>
            ))}
          </div>
        </div>
          ...
          <div className="flex flex-col lg:flex-row items-stretch gap-0">
            {[
              ...
              <div key={stage.label} className="flex-1 flex items-stretch">
                <div className={`${stage.color} text-white p-6 flex-1 ${i === 0 ? 'rounded-l-2xl' : ''} ${i === arr.length - 1 ? 'rounded-r-2xl' : ''}`}>
                  <p className="text-xs font-bold uppercase tracking-wider opacity-70 mb-3">{stage.label}</p>
        ...
        <div className="container-xl">
          ...
          <div className="space-y-6">
            {STRATEGIC_GOALS.map(goal => (
              <Card key={goal.number} className="p-6 flex items-start gap-6">
                <span className="font-mono text-4xl font-bold text-brand-200 dark:text-brand-800 flex-shrink-0">{goal.number}</span>
                <div className="flex-1">
                  <h3 className="font-display font-bold text-lg mb-2">{goal.title}</h3>
                  <p className="text-neutral-600 dark:text-neutral-400 text-sm leading-relaxed">{goal.description}</p>
                </div>
